...
export function ChatInner({ sessionId }: { sessionId: string }) {
  ...
  // Show lead capture after 3 user messages, unless already captured or dismissed.
  // Derived during render to avoid an extra state-syncing effect.
  const userMessageCount = messages.filter((m) => m.role === "user").length;
  const showLeadForm = userMessageCount >= 3 && !leadCaptured && !formDismissed;
